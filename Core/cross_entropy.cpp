...
double categoricalCrossEntropyFromLogits_OneHotY(int classIndex, const double* logits, double* logProbsWorkspace, int numClasses)
{
    vector_math::logSoftMax(logits, logProbsWorkspace, numClasses);
    return -logProbsWorkspace[classIndex];
}

void softmaxCrossEntroGrad_OneHotY(const double* logits, int classIndex, double* gradient, double* probsWorkspace, int numClasses)
{
