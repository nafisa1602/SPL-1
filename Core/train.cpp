...
void saveModel(const char* filename, const double* fgW, const double* igW, const double* ogW, const double* candW,const double* fgB, const double* igB, const double* ogB, const double* candB,
               int hiddenSize, int concatSize,const double* classifierW, const double* classifierB, int numClasses)
{
...
static int evaluateDataset(const std::vector<std::vector<int>>& X, const std::vector<int>& y, lstmState* state, const double* hZero, const double* cZero,
    int hiddenSize, int concatSize, int vocabSize, int numClasses, const double* fGateWeight, const double* fGateBias, const double* iGateWeight, 
    const double* iGateBias, const double* oGateWeight, const double* oGateBias, const double* canGateWeight, const double* canGateBias,
    dense::denseLayer& classifier, std::vector<double>& x, std::vector<double>& logits)
{
    int correct = 0;
    for (int sampleIdx = 0; sampleIdx < (int)X.size(); sampleIdx++)
    {
        ...
        for (int tt = 0; tt < Teff; tt++)
        {
            ...
            vector_math::vectorFill(state[tt].forget,    hiddenSize, 0.0);
            vector_math::vectorFill(state[tt].inputGate, hiddenSize, 0.0);
            vector_math::vectorFill(state[tt].outputGate,hiddenSize, 0.0);
            vector_math::vectorFill(state[tt].candidate, hiddenSize, 0.0);
            vector_math::vectorFill(state[tt].concat,    concatSize, 0.0);
        }
        for (int tt = 0; tt < Teff; tt++)
        {
            vector_math::vectorFill(x.data(), vocabSize, 0.0);
            ...
            lstmForward(x.data(), hPrev, cPrev, fGateWeight, fGateBias, iGateWeight, iGateBias, oGateWeight, oGateBias, canGateWeight, canGateBias, 
...
int main()
{
    ...
    lstmState state[T];
    for (int tt = 0; tt < T; tt++) initLstmState(state[tt], hiddenSize, concatSize);
...
// Initialise best_* to starting weights so spike detector always has valid weights to restore
    vector_math::vectorCopy(forgetGateWeight, best_fgW, hiddenSize * concatSize);
    vector_math::vectorCopy(inputGateWeight,  best_igW, hiddenSize * concatSize);
    vector_math::vectorCopy(outputGateWeight, best_ogW, hiddenSize * concatSize);
    vector_math::vectorCopy(candidateWeight,  best_cW,  hiddenSize * concatSize);
    vector_math::vectorCopy(forgetGateBias,   best_fgB, hiddenSize);
    vector_math::vectorCopy(inputGateBias,    best_igB, hiddenSize);
    vector_math::vectorCopy(outputGateBias,   best_ogB, hiddenSize);
    vector_math::vectorCopy(candidateBias,    best_cB,  hiddenSize);
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        for (int step = 0; step < (int)X_train.size(); step++) 
        {
            ...
            for (int tt = 0; tt < Teff; tt++) 
            {
                ...
                lstmForward(x.data(), hPrev, cPrev, forgetGateWeight, forgetGateBias, inputGateWeight,  inputGateBias,
                    outputGateWeight, outputGateBias, candidateWeight,  candidateBias, inputSize, hiddenSize, state[tt]);
            }
            vector_math::vectorCopy(state[Teff-1].hidden, hiddenDropped.data(), hiddenSize);
            ...
            if (loss != loss || loss > 1e6) 
            {
                ...
                for (int i = 0; i < numClasses; i++) std::cout << logits[i] << (i + 1 < numClasses ? "," : "");
                std::cout << "]\n";
            ...
            {
                std::cout << "[SPIKE] epoch=" << epoch << " step=" << step << " y=" << y << " loss=" << loss << " logits=";
                for (int i = 0; i < numClasses; i++) std::cout << logits[i] << " ";
                std::cout << "\n";
            }
            totalLoss += loss;
            usedSteps++;
            cross_entropy::softmaxCrossEntroGrad_OneHotY( logits.data(), y, dLogits.data(), probs.data(), numClasses);
            ...
            for (int i = 0; i < hiddenSize; i++) dHiddenT[i] *= dropoutMask[i];
            lstmBackwardTruncated(state, Teff, minInt(truncK, Teff), cZero.data(), dHiddenT.data(), forgetGateWeight, forgetGateBias, 
            inputGateWeight,  inputGateBias, outputGateWeight, outputGateBias,candidateWeight,  candidateBias, inputSize, hiddenSize, currentLR, gradClip);
            for (int i = 0; i < hiddenSize * concatSize; i++) 
            {
        ...
        std::cout << "[DEBUG] Epoch " << epoch << " | usedSteps=" << usedSteps << " | totalLoss=" << totalLoss << " | avgLoss=" << avgLoss
                  << " | maxWeight=" << maxWeightSeen << " | maxHidden=" << maxHiddenSeen << "\n";
        ...
        if (avgLoss > prevAvgLoss * 1.5) 
        {
            ...
            vector_math::vectorCopy(best_fgW, forgetGateWeight, hiddenSize * concatSize);
            vector_math::vectorCopy(best_igW, inputGateWeight,  hiddenSize * concatSize);
            vector_math::vectorCopy(best_ogW, outputGateWeight, hiddenSize * concatSize);
            vector_math::vectorCopy(best_cW,  candidateWeight,  hiddenSize * concatSize);
            vector_math::vectorCopy(best_fgB, forgetGateBias,   hiddenSize);
            vector_math::vectorCopy(best_igB, inputGateBias,    hiddenSize);
            vector_math::vectorCopy(best_ogB, outputGateBias,   hiddenSize);
            vector_math::vectorCopy(best_cB,  candidateBias,    hiddenSize);
            patienceCounter++;
        } 
        else 
        {
        ...
        int correctTest = evaluateDataset( X_test, y_test, state, hZero.data(), cZero.data(),hiddenSize, concatSize, vocabSize, numClasses,
            forgetGateWeight, forgetGateBias, inputGateWeight, inputGateBias, outputGateWeight, outputGateBias, candidateWeight, candidateBias,
            classifier, x, logits);
        double testAcc = (double)correctTest / X_test.size();
        std::cout << "Epoch " << epoch << " | LR: " << currentLR << " | Loss: " << avgLoss << " | TrainAcc: " << trainAcc << " | TestAcc: "  << testAcc;
        if (testAcc > bestTestAcc) 
        {
            ...
            vector_math::vectorCopy(outputGateBias,   best_ogB, hiddenSize);
            vector_math::vectorCopy(candidateBias,    best_cB,  hiddenSize);
            saveModel("best_model.bin",forgetGateWeight,inputGateWeight,outputGateWeight,candidateWeight,forgetGateBias,inputGateBias,outputGateBias,   
                candidateBias,hiddenSize, concatSize, classifier.weight, classifier.bias, numClasses);
            std::cout << " NEW BEST!";
        } 
        else 
        {
