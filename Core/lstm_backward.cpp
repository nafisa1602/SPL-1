...
static double computeGradientNorm(const std::vector<double>& gFW, const std::vector<double>& gIW,const std::vector<double>& gOW, 
    const std::vector<double>& gCW,const std::vector<double>& gFB, const std::vector<double>& gIB,const std::vector<double>& gOB, 
    const std::vector<double>& gCB)
{
...
void lstmBackwardTruncated(const lstmState* state, int timeSteps, int truncationWindow,const double* cellZero, const double* hiddenGradAtT,
    double* forgetGateWeight, double* forgetGateBias,double* inputGateWeight, double* inputGateBias,double* outputGateWeight, double* outputGateBias,
    double* candidateWeight, double* candidateBias,int inputSize, int hiddenSize,double learningRate, double gradClip)
{
    const int concatSize = inputSize + hiddenSize;
    const int wSize = hiddenSize * concatSize;
    static std::vector<double> gradForgetW; //allocated once and reuded
    ...
    if ((int)dConcat.size() != concatSize) dConcat.assign(concatSize, 0.0);
    else for (int j = 0; j < concatSize; j++) dConcat[j] = 0.0;
    ...
    for (int time = timeSteps - 1; time >= tEnd; time--)
    {
        ...
        for (int i = 0; i < hiddenSize; i++)
        {
            ...
            for (int j = 0; j < concatSize; j++)
            {
                ...
                dConcat[j] += forgetGateWeight[idx] * dF_pre + inputGateWeight[idx]  * dI_pre + outputGateWeight[idx] * dO_pre + candidateWeight[idx]  * dG_pre;
            }
        ...
        for (int j = 0; j < hiddenSize; j++) dHiddenPrev[j] = dConcat[inputSize + j];
        for (int i = 0; i < hiddenSize; i++)
        {
    ...
    double gradNorm = computeGradientNorm(gradForgetW, gradInputW, gradOutputW, gradCandW, gradForgetB, gradInputB, gradOutputB, gradCandB);
